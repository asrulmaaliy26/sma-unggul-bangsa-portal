        ...
        <div className="flex justify-between h-20">
          <div className="flex items-center gap-6">
            <Link to="/" className="flex items-center space-x-3 group">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Lembaga Unggul Bangsa</span>
              </div>
            </Link>
          ...
          <div className="hidden lg:flex items-center space-x-1">
            ...
            <div className="relative" onMouseEnter={() => setIsAboutOpen(true)} onMouseLeave={() => setIsAboutOpen(false)}>
              <button 
                className={`flex items-center px-4 py-2.5 rounded-xl font-bold text-sm transition-all duration-300 ${
        ...
        <div className="lg:hidden bg-white border-t border-slate-100 py-6 px-4 space-y-4 animate-fadeIn">
          ...
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className={`block px-4 py-3 text-sm font-black rounded-xl ${
                isActive(link.path) ? `${currentTheme.bg} text-white` : 'text-slate-700 bg-slate-50'
              }`}

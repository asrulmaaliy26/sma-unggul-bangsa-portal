...
const About: React.FC = () => {
  ...
  const renderVisiMisi = () => (
    <div className="space-y-16 animate-fadeIn">
      <div className={`${theme.bg} p-12 md:p-20 rounded-[4rem] text-white relative overflow-hidden shadow-2xl transition-colors duration-500`}>
        <div className="relative z-10 text-center max-w-4xl mx-auto">
          <p className="arabic-text text-5xl text-islamic-gold-500 mb-10">إِنَّ اللَّهَ مَعَ الَّذِينَ اتَّقَوْا وَالَّذِينَ هُمْ مُحْسِنُونَ</p>
          <h2 className="text-2xl font-black uppercase tracking-[0.4em] mb-8 text-islamic-gold-500/80">
            {isYayasan ? 'Visi Yayasan' : `Visi ${activeLevel}`}
          </h2>
          <p className="text-3xl md:text-5xl leading-tight font-black mb-10">
            "{content.visi}"
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div className="bg-islamic-gold-500 p-12 rounded-[3.5rem] text-white shadow-xl shadow-islamic-gold-100 flex flex-col justify-center overflow-hidden relative">
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-2 gap-4">
              {['Amanah', 'Fathanah', 'Siddiq', 'Tabligh'].map(val => (
                <div key={val} className="bg-white/10 backdrop-blur-md p-6 rounded-2xl border border-white/20 hover:bg-white/20 transition-all">
                  <p className="font-black text-xl mb-1">{val}</p>
  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      <header className={`${theme.bg} py-24 px-4 text-center relative overflow-hidden transition-colors duration-500`}>
        <div className="max-w-4xl mx-auto relative z-10">
          <h1 className="text-4xl md:text-6xl font-black text-white mb-6 leading-tight animate-fadeIn">{getTitle()}</h1>
          <p className="text-white/80 text-lg md:text-xl font-medium animate-fadeIn">
